...
// Parsed resume facts and text also live on the relational profile so every
// surface can reuse them. Original file bytes and generated documents remain
// in the document store layered on top.
...
function emptyProfile() {
  return {
    ...
    documents: {},     // parsed reusable document relations, such as resume text + facts
    updatedAt: null
  };
}
...
export async function loadProfile() {
  ...
  for (const key of ["identity", "links", "work", "preferences", "answers", "domains", "documents"]) {
...
export function profileSummaryText(profile) {
  ...
  if (resume?.summary?.skills?.length) lines.push(`Resume skills: ${resume.summary.skills.slice(0, 8).join(", ")}`);
  if (resume?.fileName) lines.push(`Resume on file: ${resume.fileName}`);
